...
The launch pack writes the env template, production env checklist, current readiness action report, completion audit, provider launch plan, and ordered Supabase SQL bundle into one operator folder. It is still placeholder-only; do not commit filled secrets or provider dashboard values.
...
Check Upstash rate limiting, hashed Redis keys, pipeline requests, emergency memory fallback behavior, and API route coverage for abuse-sensitive endpoints:
...
Run the same daily-variation gate through live OpenAI for a focused same-user date sweep:
...
If the repository secret `OPENAI_API_KEY` is configured, the workflow can also run `npm run soul:quality:ai`, `npm run soul:daily:ai`, `npm run astro:quality:ai`, `npm run more-guidance:quality:ai`, and `npm run shani:quality:ai` for live prompt-quality regression checks. The secret is used only inside the CI job and is not bundled into the frontend or APK.
...
The settings drawer shows the configured Clerk secure-session state and exposes account/sign-out controls so operators can confirm the browser or APK has an authenticated session before strict backend auth is enabled.
...
The response also includes `providerSummary` and a secret-safe `providers` matrix for the planning-image stack. Deployed smoke tests and backend-connected APK builds require the exact planned provider IDs, so stale or partial backends are rejected before release.
...
```
Production readiness and runtime delivery require the SMS webhook URL to be a real HTTPS provider URL and the SMS webhook token to be present, non-placeholder, and at least 16 characters long. The backend sends it to the SMS webhook as a Bearer token.
...
The local app can show a demo OTP when Supabase/SMS delivery is unavailable. Production builds must keep both `OTP_DEMO_ENABLED=false` and `VITE_LOCAL_AUTH_FALLBACK=false`; strict public env checks fail if local OTP fallback is enabled for release. In production, configure Supabase plus a token-authenticated SMS webhook for phone OTP delivery. For authenticated API protection, configure Clerk and set:
...
Use [docs/production-launch-runbook.md](docs/production-launch-runbook.md) as the short provider-by-provider launch path. Current completion status is audited in [docs/production-completion-audit.md](docs/production-completion-audit.md). The latest secret-safe live AI quality evidence is in [docs/live-ai-quality-report.md](docs/live-ai-quality-report.md), and the runtime tab walkthrough is in [docs/runtime-ui-qa-report.md](docs/runtime-ui-qa-report.md). The longer roadmap remains in [docs/production-roadmap.md](docs/production-roadmap.md).
...
- Run `npm run soul:quality:ai`, `npm run soul:daily:ai`, `npm run astro:quality:ai`, `npm run more-guidance:quality:ai`, and `npm run shani:quality:ai` with `OPENAI_API_KEY` configured before release after prompt changes.
