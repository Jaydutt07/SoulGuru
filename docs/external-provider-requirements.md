...
- `OTP_SMS_WEBHOOK_URL`: SMS delivery webhook/API endpoint.
- `OTP_SMS_WEBHOOK_TOKEN`: server-only token for the SMS webhook.
...
Actions:
- Choose an SMS provider or webhook.
- Configure the webhook to send the OTP body generated by the backend.
- Verify:
