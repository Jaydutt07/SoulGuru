...
function pickSpecificRoot(parts, seed = 0) {
  ...
  const routes = {
    ...
    "friend-cancelling": [
      `The root is uneven availability. A closest friend cancelling plans but expecting emotional access has made ${name}'s care easy to call on and easy to postpone. That creates hurt because the friendship has warmth without balanced effort. The issue is not one cancelled plan; it is the repeated rule underneath it. Care needs reciprocity: who makes time, who repairs absence, and whether support is still mutual when someone is upset.`,
    ...
    "parent-support": [
      `The root is monthly support being decided by emotion instead of a visible budget. ${name} wants to help parents, but fear and resentment grow when support has no ceiling, review date, or emergency category. The problem is not love. It is duty without accounting. A loving plan must show what is regular, what is temporary, and what cannot be carried alone without damaging the future you are also responsible for.`,
    ...
    "panic-presentation": [
      `The root is the body reading presentation pressure as danger before the mind can use what it knows. Chest tightness is not random; it is the physical alarm arriving before ${name} has a reliable pre-speaking sequence. The problem is not lack of preparation. It is that the body has not been given a repeatable opening ritual. Confidence will follow sequence: breath, first slide, first sentence, and one grounded pause.`,
...
function pickCategoryRoot(parts, seed = 0) {
  ...
  const routes = {
    ...
    "study-health": [
      `The root is the body carrying pressure that the mind keeps postponing. Around ${cue}, discipline has become mixed with fear, so rest, food, or preparation starts feeling like another performance. Anxiety is not the enemy; it is an alarm that rhythm has become too negotiable. ${name} needs to rebuild sequence before demanding confidence: body first, then timed effort, then review.`,
