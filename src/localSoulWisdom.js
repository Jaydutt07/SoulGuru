...
function compactAvoid(text, seed = 0) {
  ...
  if (normalized.includes("checking") || normalized.includes("signs")) {
    return pickLine([
      ...
      "reading small shifts as verdicts",
...
function avoidPhrase(text, seed = 0) {
  ...
  if (normalized.includes("checking") || normalized.includes("signs")) {
    return pickLine([
      ...
      "reading each small shift as a verdict",
...
function sceneStatementSubject(scene, seed = 0) {
  ...
  if (/\b(desk|workspace)\b/.test(normalized)) {
    return pickLine([
      "Desk corner pressure",
      "The workspace edge",
      "The visible desk reset"
    ], seed, scene);
  }
...
function workAndCloseLine(work, close, seed = 0) {
  ...
  return pickLine([
    ...
    `${capitalize(gerund)} belongs before the next private debate begins.`
...
function timedNameActionLine(name, edgeInstruction, seed = 0) {
  ...
  return pickLine([
    ...
    `The answer gets clearer for ${name} when the urge to ${edgeInstruction} has a time limit, not another courtroom.`,
