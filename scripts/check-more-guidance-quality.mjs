...
function isLowQualityPaidGuidance(text) {
  ...
  return [
    ...
    /\bmake protect\b/,
    /\buse protecting\b/,
    /\bletting a practical duty become emotional negotiation\b/,
    /\blet listening stay present without becoming rescue\b/,
    /\blisten warmly without making repair your automatic assignment\b/,
    /\blisten, then choose only the repair that truly belongs to you\b/,
    /\bthe conversation practical before it gets crowded\b/,
    /\bproof before sleep so tomorrow compares behavior\b/,
    /\bbefore the difficult exchange, write one sentence\b/,
    /\bwaiting for outside softness before honoring the\b/,
    /\breview should show which pressure became easier\b/,
    /\bbe repeated without turning into a performance\b/,
    /\bthe work a boundary before explanation expands\b/,
    /\bmeasure what changed through proof\b/,
    /\bmeet tomorrow before the day starts negotiating\b/,
    /\bvisible piece before another promise gets added\b/,
    /\bboundary, reply, or schedule change that interrupts\b/,
    /\blet relationships follow let\b/,
    /\blet work follow let\b/,
    /\blet let\b/,
    /\blet do not\b/,
