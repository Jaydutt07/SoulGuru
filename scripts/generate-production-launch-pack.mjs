...
function buildReadme() {
  return [
    ...
    "4. Activate GitHub Actions only with workflow-capable credentials: run `npm run ci:install-workflow`, `npm run ci:install-workflow -- --check`, and push `.github/workflows/ci.yml` after the GitHub token or SSH key can update workflow files.",
    "5. Run `npm run production:actions` and `npm run production:audit` with production env loaded.",
    "6. Run `npm run production:check` with production env loaded.",
    "7. Run `npm run android:security:check` before mobile release output.",
    "8. Run `npm run android:artifact:check -- --expect-url=https://your-production-domain.app` after building the APK.",
    "9. Run `npm run production:domain:smoke -- --expect-ready` after DNS and Vercel custom-domain setup are live.",
    "10. Run `npm run release:check -- --url=https://your-production-domain.app --include-ai --include-android-signing` before release.",
    "11. After launch, run `npm run soul:feedback:report` before prompt changes so real missed-reading themes guide tuning.",
    "",
    "## Safety",
    "",
    ...
    "- GitHub workflow activation requires a credential with `workflow` scope; keep `docs/github-actions-ci.yml` as source of truth until then.",
